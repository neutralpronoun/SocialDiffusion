...
class FBH2Dataset(InMemoryDataset):
    ...
    def process(self):
        ...
        for i, G in enumerate(tqdm(suppl)):
            ...
            for edge in list(G.edges()):
                ...
                # if (weight - mean_weight) / std_weight <= -0.5:
                #     etype = [1]
                # elif (weight - mean_weight) / std_weight >= 0.5:
                #     etype = [3]
                # else:
                #     etype = [2]
                etype = [1]
...
class FBH2DataModule(MolecularDataModule):
    ...
    def prepare_data(self) -> None:
        ...
        datasets = {'train': FBH2Dataset(stage='train',
                                        root=root_path,
                    ...
                    'val': FBH2Dataset(stage='val',
                                      root=root_path,
                                      remove_h=self.cfg.dataset.remove_h,
                                      max_size = self.cfg.dataset.max_size,
                                        n_workers = self.cfg.train.num_workers),
                    'test': FBH2Dataset(stage='test',
                                       root=root_path,
                                       remove_h=self.cfg.dataset.remove_h,
                                       max_size = self.cfg.dataset.max_size,
                                        n_workers = self.cfg.train.num_workers)}
