...
class FBDataset(InMemoryDataset):
    ...
    def download(self):
        ...
        self.G = G
        print(f"Finding communities {self.n_reps} times, with a resolution of {self.resolution}")
        ...
        partition_df = pd.DataFrame({'community_id':[i for i in range(len(self.partition))]})
    ...
    def communities_split(self, G):
        ...
        if self.partition == {}:
            ...
            max_partition_so_far = max(list(self.partition.keys()))

        for p in partition_dict:
            self.partition[p + max_partition_so_far] = partition_dict[p]
...
class FBDataModule(MolecularDataModule):
    ...
    def prepare_data(self) -> None:
        ...
        datasets = {'train': FBDataset(stage='train',
                                        root=root_path,
                    ...
                    'val': FBDataset(stage='val',
                                      root=root_path,
                                      remove_h=self.cfg.dataset.remove_h,
                                      max_size = self.cfg.dataset.max_size),
                    'test': FBDataset(stage='test',
                                       root=root_path,
                                       remove_h=self.cfg.dataset.remove_h,
                                       max_size = self.cfg.dataset.max_size)}
